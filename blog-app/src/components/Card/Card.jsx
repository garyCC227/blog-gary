import React from "react";
import {useHistory} from 'react-router-dom';

import styles from "./Card.module.css";
import post from "../../blogPost.json";
import cardData from './cardNote.json'


const Card = ({}) => {

  let history = useHistory();
  /** 
  const redirect = ()=>{
    history.push("/post/1588270500");
  }
  **/
  return( 
    
    <div>
      {
        cardData.map( (cardDetail, index) =>{
          //console.log(cardDetail.tag)
          let tag = cardDetail.tag
          //console.log(tag)
        //console.log('number of index: '+ cardData.length)
        let ID  = cardDetail.id;
        /*
        for (let j = 0; j<cardData.length; j++){
          //console.log('number of index: '+j)
          for (let i = 0; i < cardData[j].tag.length; i++){
            //console.log(cardData[j].tag[i])
          }
        }*/
        
          return(
            <div class={`ui card ${styles["card-size"]}`} onClick={()=>{history.push('/post/'+ ID);}}>
            <div class='content'>
          
           
           {
            tag.map((tagData)=>{
              return (
                //console.log(tagData)
                  <a class={`ui pink label ${styles["my-label"]}`}>{tagData}</a>
                
                
              )
            })
           
           
        }

        <div className={styles["my-header"]}>
          {cardDetail.title}
        </div>

        <div class={styles["my-desc"]}>
          <p>
            Veronika Ossi is a set designer living in New York who enjoys
            kittens, music, and partying.Veronika Ossi is a set designer living
            in New York who enjoys kittens, music, and partying.Veronika Ossi is
            a set designer living in New York who enjoys kittens, music, and
            partying.
          </p>
        </div>

        <div class='header'>
          <img
            class='left floated ui image'
            width="50"
            height="50"
            src='data:image/svg+xml;base64,'
            alt='profile_img'
          />
          <span class='header'>
            Linchen
            <div class='meta' style={{marginTop:'5px'}}>
              <span class='time'>May 07, 2020 </span>
              <span class='right floated category'><i class="clock outline icon" style={{marginRight:'5px'}}/>10 mins</span>
            </div>
          </span>
        </div>
      </div>
    </div>
          )

        }
        )
        }

     
    </div>




   
  )


/** 
  return (
    <div class={`ui card ${styles["card-size"]}`} onClick={redirect}>
      
      <div class='content'>
        <div>
          <a class={`ui pink label ${styles["my-label"]}`}>Pink</a>
          <a class={`ui teal label ${styles["my-label"]}`}>Teal</a>
          <a class={`ui blue label ${styles["my-label"]}`}>Blue</a>
        </div>
        <div className={styles["my-header"]}>
          What is git and why we should have a knowledge of it.
        </div>

        <div class={styles["my-desc"]}>
          <p>
            Veronika Ossi is a set designer living in New York who enjoys
            kittens, music, and partying.Veronika Ossi is a set designer living
            in New York who enjoys kittens, music, and partying.Veronika Ossi is
            a set designer living in New York who enjoys kittens, music, and
            partying.
          </p>
        </div>

        <div class='header'>
          <img
            class='left floated ui image'
            width="50"
            height="50"
            src='data:image/svg+xml;base64,'
            alt='profile_img'
          />
          <span class='header'>
            Linchen
            <div class='meta' style={{marginTop:'5px'}}>
              <span class='time'>May 07, 2020 </span>
              <span class='right floated category'><i class="clock outline icon" style={{marginRight:'5px'}}/>10 mins</span>
            </div>
          </span>
        </div>
      </div>
    </div>
  );**/
};


export default Card;
